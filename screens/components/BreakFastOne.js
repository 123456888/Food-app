...
const BreakFastOne = () => {
  ...
  const renderItems = ({ item }) => {
    if (input === "") {
      return (
        <Animated.View entering={FadeInDown.duration(500).springify()} className="mt-5">
          <TouchableOpacity className="px-3 items-center" onPress={() => DataDetails(item)}>
            <Image source={{ uri: item.strMealThumb }} style={{ height: 155, width: 155 }} className="rounded-full"></Image>
            <Text numberOfLines={2} style={{ width: 100 }} className="mb-4 text-red-400 text-sm">{item.strMeal}</Text>
          </TouchableOpacity>
        </Animated.View>
      )
    }
    ...
    <View className="flex-1 py-10">
      <ScrollView className="space-y-6">
        ...
        <View className="h-full">
          ...
          <Text className="text-red-400 text-3xl px-3 mt-4 font-bold">Break Fast</Text>
          <View>
            <FlatList
              data={BreakFast}
